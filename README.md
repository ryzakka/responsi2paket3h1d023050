...
_loadUserData(): Fungsi ini dipanggil saat halaman dimuat untuk mengambil nama lengkap (full_name) dari pengguna yang sedang login dari tabel member dan menampilkannya di AppBar sebagai sapaan selamat datang.•_loadBooks(): Memuat ulang data buku dari Supabase. Fungsi ini dipanggil saat halaman pertama kali dibuka, setelah buku baru ditambahkan, atau saat pengguna melakukan "pull-to-refresh".•_getBooks(): Fungsi inti yang melakukan permintaan select ke tabel buku di Supabase untuk mengambil semua data inventaris buku, mengurutkannya berdasarkan tanggal pembuatan terbaru.•_deleteBook(id): Mengirim permintaan delete ke Supabase untuk menghapus buku berdasarkan id-nya.•_filterBooks(): Fungsi ini berjalan setiap kali pengguna mengetik di kolom pencarian. Ia akan menyaring daftar buku yang ditampilkan berdasarkan judul, penulis, atau penerbit yang cocok dengan teks pencarian. <br>
